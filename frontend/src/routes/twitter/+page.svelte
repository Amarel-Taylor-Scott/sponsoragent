<script>
  ...
  async function joinWaitlist() {
    error = ''; loading = true;
    try {
      const res = await fetch('/api/v1/waitlist', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ email, variant: 'A', source: 'twitter_landing' }) });
...
<svelte:head>
  <title>Twitter/X Sponsorships | SponsorAgent</title>
  <meta name="description" content="Find brand sponsors for your Twitter/X account. SponsorAgent matches accounts with brands based on audience and niche." />
</svelte:head>
<div class="platform-page container">
  <div class="platform-hero">
    <span class="platform-badge" style="background: rgba(29,161,242,0.15); color: #1da1f2">Twitter / X</span>
    <h1>Find Sponsors for Your <span class="gradient-text">Twitter/X Account</span></h1>
    <p>SponsorAgent analyzes your tweets, engagement, and audience to match you with brands looking for Twitter/X influencers.</p>
    <div class="features">
      <div class="feature card"><h3>Tweet Analysis</h3><p>We analyze your tweet engagement, thread performance, impressions, and audience quality to score sponsor readiness.</p></div>
      <div class="feature card"><h3>Thread Sponsorships</h3><p>Long-form threads are valuable to brands. We match you with sponsors looking for educational or commentary threads.</p></div>
      <div class="feature card"><h3>Niche Influence</h3><p>Twitter/X influence is niche-specific. We identify your areas of authority and match accordingly.</p></div>
    </div>
...
<style>
  ...
  .gradient-text { background: linear-gradient(135deg, #1da1f2, #5bc0ff); -webkit-background-clip: text; -webkit-text-fill-color: transparent; }
