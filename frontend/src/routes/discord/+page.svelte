<script>
  ...
  async function joinWaitlist() {
    error = ''; loading = true;
    try {
      const res = await fetch('/api/v1/waitlist', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ email, variant: 'A', source: 'discord_landing' }) });
...
<svelte:head>
  <title>Discord Community Sponsorships | SponsorAgent</title>
  <meta name="description" content="Monetize your Discord community. Find brand sponsors for your Discord server with SponsorAgent." />
</svelte:head>
<div class="platform-page container">
  <div class="platform-hero">
    <span class="platform-badge" style="background: rgba(88,101,242,0.15); color: #5865f2">Discord</span>
    <h1>Monetize Your <span class="gradient-text">Discord Community</span></h1>
    <p>SponsorAgent helps Discord server owners find brand partnerships based on community size, engagement, and niche.</p>
    <div class="features">
      <div class="feature card"><h3>Community Analysis</h3><p>We evaluate your server's member count, activity levels, and niche to match you with relevant brands.</p></div>
      <div class="feature card"><h3>Brand Partnerships</h3><p>Access brands looking to sponsor Discord communities through custom roles, channels, and event partnerships.</p></div>
      <div class="feature card"><h3>Engagement Metrics</h3><p>Showcase your community engagement data to prove value to potential sponsors.</p></div>
    </div>
...
<style>
  ...
  .gradient-text { background: linear-gradient(135deg, #5865f2, #8b99ff); -webkit-background-clip: text; -webkit-text-fill-color: transparent; }
