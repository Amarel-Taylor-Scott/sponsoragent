  ...
  async function joinWaitlist() {
    ...
    try {
      const res = await fetch('/api/v1/waitlist', {
        ...
        body: JSON.stringify({ email, variant: 'A', source: 'twitch_landing' }),
      });
...
<svelte:head>
  <title>Twitch Sponsorship Manager | SponsorAgent</title>
  <meta name="description" content="Find Twitch sponsors for your stream. SponsorAgent matches streamers with gaming and lifestyle brands." />
</svelte:head>

<div class="platform-page container">
  <div class="platform-hero">
    <span class="platform-badge" style="background: rgba(145,70,255,0.15); color: #9146ff">Twitch</span>
    <h1>Find Sponsors for Your <span class="gradient-text">Twitch Stream</span></h1>
    <p>SponsorAgent analyzes your Twitch channel metrics and matches you with gaming, tech, and lifestyle brands looking for streamers.</p>

    <div class="features">
      <div class="feature card">
        <h3>Stream Analysis</h3>
        <p>We analyze your average viewers, follower growth, stream categories, and chat engagement to build your sponsor profile.</p>
      </div>
      <div class="feature card">
        <h3>Gaming Brand Database</h3>
        <p>Access our curated database of gaming peripherals, energy drinks, VPNs, and tech brands actively sponsoring Twitch streamers.</p>
      </div>
      <div class="feature card">
        <h3>Rate Calculator</h3>
        <p>Know your worth. Our rate recommendations factor in concurrent viewers, subscriber count, and engagement quality.</p>
      </div>
    </div>
    ...
    <div class="cta-section card">
      ...
      {#if submitted}
        <div class="alert-success">You're on the list!</div>
      {:else}
...
<style>
  ...
  .gradient-text { background: linear-gradient(135deg, #9146ff, #bf8dff); -webkit-background-clip: text; -webkit-text-fill-color: transparent; }
